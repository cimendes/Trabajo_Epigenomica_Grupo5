...
# ./Apartado_2_ANOTACIONES/9.2.Obtencion_frecuencias_segmentos_todos_estados.sh


# 1. Obtención de todas las etiquetas con las que se han anotado los segmentos
tail -n +2 ./Apartado_2_ANOTACION/ARCHIVOS_ANOTACIONES/Anotaciones_interseccion_todos_chr_todos_estados.csv | cut -d "," -f 16 | sort | uniq > ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Etiquetas_anotaciones.txt
...
do
  tail -n +2 ./Apartado_2_ANOTACION/ARCHIVOS_ANOTACIONES/Anotaciones_interseccion_todos_chr_todos_estados.csv | sort -t "," -k 1,1 -k 2,2 -k 3,3 | grep "$line" | cut -d "," -f 1,2,3 | uniq | wc -l >> ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Valores_anotaciones.txt

done < "$input"

# 3. Unión de los resultados de los apartados 1 y 2
paste -d "," ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Etiquetas_anotaciones.txt ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Valores_anotaciones.txt > ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Etiquetas_Valores_anotaciones_todos_estados.csv

# 4. Eliminación de ficheros temporales
rm ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Valores_anotaciones.txt
rm ./Apartado_2_ANOTACION/ARCHIVOS_PARA_GRAFICA/Etiquetas_anotaciones.txt
